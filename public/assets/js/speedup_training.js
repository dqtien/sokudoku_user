...
var SpeedupTraining = function () {
  function SpeedupTraining() {
    ...
    this.isStopTimerView = ko.observable(true);
    this.speedupCount = 2;
    this.speedupTime = 90;
    this.isOver180 = false;
    ...
    this.model = {
      ...
      times: {
        remainingTime: ko.observable('60:00'),
        initialTime: ko.observable(5)
      }
    };
  }
  ...
  _createClass(SpeedupTraining, [{
    ...
    value: function readingSpeed() {
      ...
      return singleLine * lineInfo / point * tempo;
    }
  }, {
    ...
    value: function stopTimer() {
      ...
      speedupTraining.speedupCount = 1;
    ...
    value: function nextTime() {
      if (speedupTraining.model.tempoCount.logic() >= 120 && speedupTraining.isStopTimerView()) {
        speedupTraining.isStopTimerView(false);
        speedupTraining.model.times.remainingTime('5:00');
      }

      if (speedupTraining.model.tempoCount.logic() >= 180) {
        this.isOver180 = true;
      }
      if (this.isOver180) {
        this.speedupTime = 10;
      ...
      if (speedupTraining.speedupCount >= this.speedupTime) {
        tempoSlider.up(true);
        speedupTraining.speedupCount = 1;
      } else {
        // 2分毎に上げるための数字
        speedupTraining.speedupCount++;
      }
      ...
      if (min === 0) {
        if (sec === 0) {
          if (speedupTraining.isFirstTime()) {
            speedupTraining.isFirstTime(false);
            speedupTraining.setInitialTime();
            speedupTraining.isTraining(false);
            var cT = speedupTraining.model.tempoCount.logic();
            speedupTraining.model.tempoCount.logic(cT - 40);
            speedupTraining.model.tempoCount.view(cT - 40);
            this.stopTimer();

            min = 5;
          } else {
            alert('\u7D42\u4E86\u3057\u307E\u3057\u305F\u3002\n\u4ECA\u56DE\u306F\u6BCE\u5206' + this.readingSpeed() + '\u6587\u5B57\u3067\u3057\u305F\uFF01');
            speedupTraining.isFirstTime(true);
            speedupTraining.isStopTimerView(true);
            speedupTraining.model.times.remainingTime('5:00');
            this.stopTimer();
          }
        } else {
...
var TempoSlider = function () {
  ...
  _createClass(TempoSlider, [{
    ...
    key: 'up',
    value: function up() {
      var fromAuto = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
